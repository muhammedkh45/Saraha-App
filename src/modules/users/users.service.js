...
export const signUp = async (req, res, next) => {
  ...
  if (!code) {
    throw new Error("Failed to send Verfication email", {cause:500});
  }
  const userData = {
    name,
    email,
...
export const confirmEmail = async (req, res, next) => {
  try {
    ...
    if (user.timeToReattempts !== 0 && user.timeToReattempts > Date.now()) {
      ...
      return res
        .status(401)
        .json({ message: `Try to reattempt after ${minutesLeft} minutes` });
    }
    if (user.code.toString() !== code.toString()) {
      if (user.attemptsLeft <= 1) {
        ...
        return res.status(401).json({
          message: `Try to reattempt, you have ${
            user.attemptsLeft - 1
          } attempts left`,
        });
      }
    ...
    if (error.name === "JsonWebTokenError") {
      ...
      if (!code) {
        return res
          .status(500)
          .json({ message: "Failed to send Verfication email" });
      }
